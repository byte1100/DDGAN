...
targan_solver.py       # Step size.
    ...
    def train_adv_both(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            # Fetch real images and labels.
            try:
targan_solver.py               x_real, label_org = next(data_iter)
            except:
                data_iter = iter(data_loader)
                x_real, label_org = next(data_iter)
    ...
    def test_attack_cond(self):
        ...
        for i, (x_real, c_org) in enumerate(data_loader):
            ...
            for idx, c_trg in enumerate(c_trg_list):
                ...
                if idx == 0:
                    # Wrong Class
                stargan_solver.py    x_adv, perturb = pgd_attack.perturb(x_real, gen_noattack, c_trg_list[0])
    ...
    def test_universal_model_level_attack(self, i, x_real, c_org, pgd_attack):
        ...
        for idx, c_trg in enumerate(c_trg_list):
            ...
            with torch.no_grad():
                ...
                # x_real_mod = self.blur_tensor(x_real_mod) # use blur
                gen_noattack, gen_noattack_feats = self.G(x_real_mod, c_trg)
           #x_adv, perturb = pgd_attack.perturb_joint_class(x_real, gen_noattack, c_trg_list, self.G)
            #adv, perturb = pgd_attack.universal_perturb_stargan_mi(x_real, gen_noattack, c_trg, self.G) 
            # Attacks
               #x_adv, perturb = pgd_attack.perturb_iter_class(x_real, gen_noattack, c_trg_list)
               # x_adv, perturb = pgd_attack.perturb_joint_class(x_real, gen_noattack, c_trg_list, self.G)  # Vanilla
    ...
    def test_universal_model_level(self, i, x_real, c_org, perturb, args):
        ...
        for idx, c_trg in enumerate(c_trg_list):
            with torch.no_grad():
                ...
                gen, gen_feats = self.G(x_adv, c_trg)
                x_fake_list.append(gen)
 s              x_noattack_list.append(gen_noattack)
