...
if __name__ == "__main__":
    ...
    if args_attack.global_settings.universal_perturbation_path:
        ...
        #up_attention = torch.load(args_attack.global_settings.attentiongan_perturbation_path)
        ...
        #pgd_attack.up = (up_attgan + up_hisd + up_aggan + up_stargan)/4 
        #up = torch.load(args_attack.global_settings.single_path)
        #pgd_attack.up = torch.clamp(up, -0.05, 0.05)
        pgd_attack.up = torch.load(args_attack.global_settings.single_path)
        #pgd_attack.up = torch.load(args_attack.global_settings.attentiongan_perturbation_path)
    print(pgd_attack.up.size())
    ...
    for idx, (img_a, att_a, c_org) in enumerate(test_dataloader):
        ...
        #pgd_attack.up.requires_grad_(False)
        #print(pgd_attack.up.size())
        ...
        #保存加水印图片
        out_file = './demo_results/AttGAN_plus.jpg'
        vutils.save_image(
            samples[1], out_file,
            nrow=1, normalize=True, range=(-1., 1.)
        )
    ...
    for idx, (img_a, att_a, c_org) in enumerate(test_dataloader):
        ...
        for j in range(len(x_noattack_list)):
        ...
        for j in range(2):
            gen = x_fake_list[j]
            out_file = './demo_results/stargan_perturbation_{}.jpg'.format(j)
            vutils.save_image(gen, out_file, nrow=1, normalize=True, range=(-1., 1.))
        break
    ...
    for idx, (img_a, att_a, c_org) in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            vutils.save_image(gen, out_file, nrow=1, normalize=True, range=(-1., 1.))
        break
    print('HiDF {} images. L1 error: {}. L2 error: {}. prop_dist: {}. L0 error: {}. L_-inf error: {}.'.format(n_samples, l1_error / n_samples, l2_error , float(n_dist) / n_samples, l0_error / n_samples, min_dist / n_samples))
